...
_EXTERNAL_MAP_PROJECT_PATH = Path(__file__).resolve().parents[4]
...
class ExternalProjectRunner:
    """Manage lifecycle of the external map-localization project."""
    ...
    def __init__(self, project_root):
        ...
        self._frontend_url = "http://127.0.0.1:5173/"
        self._backend_url = "http://127.0.0.1:9000/"
        self._log_path = self._project_root / "run-from-navisar.log"
    ...
    def start(self):
        with self._lock:
            ...
            if not self._project_root.exists() or not self._project_root.is_dir():
                return {
                    "ok": False,
                    "error": f"project directory not found: {self._project_root}",
                }
            if shutil.which("npm") is None:
                return {"ok": False, "error": "npm is not installed or not in PATH"}

            python_bin = self._project_root / ".venv" / "bin" / "python"
            if not python_bin.exists():
                python_bin = Path(sys.executable or "python3")
            script = (
                "set -e\n"
                f"cd {shlex.quote(str(self._project_root))}\n"
                f"{shlex.quote(str(python_bin))} -m uvicorn app.backend.api:app --host 0.0.0.0 --port 9000 &\n"
                "BACK_PID=$!\n"
...
class PixhawkMavlinkCsvRecorder:
    """Thread-safe CSV recorder for MAVLink messages received from Pixhawk."""
    ...
    def _session_status_locked(self):
        return {
            ...
            "stopped_at": self._stopped_at,
            "data_points": self._row_count,
            "message_types": sorted(self._msg_types),
            "csv_file": str(self._csv_path) if self._csv_path else None,
        }
    ...
    def start(self):
        with self._lock:
            if self._recording:
                return self._session_status_locked()
            stamp = time.strftime("%Y%m%d_%H%M%S")
            base_id = f"pixhawk_mavlink_csv_{stamp}"
            session_id = base_id
            idx = 1
            session_dir = self._root_dir / session_id
            while session_dir.exists():
            ...
            self._session_id = session_id
            self._session_dir = session_dir
            self._csv_path = session_dir / "pixhawk_mavlink_messages.csv"
            self._meta_path = session_dir / "session_meta.json"
            self._csv_file = self._csv_path.open("w", encoding="utf-8", newline="")
            self._csv_writer = csv.DictWriter(
                self._csv_file,
    ...
    def log_mavlink_message(self, msg, recv_ts=None):
        with self._lock:
            if not self._recording or self._csv_writer is None or self._csv_file is None:
                return
            now = float(recv_ts) if recv_ts is not None else time.time()
            msg_type = ""
            msg_dict = {}
            try:
                msg_type = str(msg.get_type() or "")
            except Exception:
                msg_type = ""
            try:
            ...
            src_system = getattr(header, "srcSystem", None) if header is not None else None
            src_component = getattr(header, "srcComponent", None) if header is not None else None
            seq = getattr(header, "seq", None) if header is not None else None
            time_usec = (
                msg_dict.get("time_usec")
                if isinstance(msg_dict, dict)
                else None
            )
            if time_usec is None and isinstance(msg_dict, dict):
                time_usec = msg_dict.get("usec")
            time_boot_ms = (
                msg_dict.get("time_boot_ms")
                if isinstance(msg_dict, dict)
                else None
            )
            if time_boot_ms is None and isinstance(msg_dict, dict):
                time_boot_ms = msg_dict.get("time_ms")
            row = {
                "server_ts": now,
                "server_timestamp_iso": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(now)),
                "msg_id": (
                    getattr(msg, "get_msgId", lambda: None)()
                    if hasattr(msg, "get_msgId")
                    else (getattr(header, "msgId", None) if header is not None else None)
                ),
                "msg_type": msg_type,
                ...
                "payload_json": json.dumps(msg_dict, separators=(",", ":"), default=str),
            }
            try:
                self._csv_writer.writerow(row)
                self._csv_file.flush()
    ...
    def download_csv(self):
        with self._lock:
            ...
            if session_dir is None or not session_dir.exists():
                return None, None
            csv_path = session_dir / "pixhawk_mavlink_messages.csv"
            if not csv_path.exists():
                return None, None
            return csv_path, f"{session_dir.name}_pixhawk_mavlink_messages.csv"
    ...
    class DashboardHandler(http.server.SimpleHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if req_path.startswith("/blackbox/status"):
                ...
                self.wfile.write(data)
                return
            if req_path.startswith("/external-project/status"):
                payload = external_project_runner.status()
                data = json.dumps(payload).encode("utf-8")
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self.send_header("Cache-Control", "no-store")
            ...
            if req_path.startswith("/sensor-csv/download"):
                ...
                self.wfile.write(blob)
                return
            if req_path.startswith("/pixhawk-mavlink-csv/status"):
                payload = pixhawk_mavlink_csv_recorder.status()
                data = json.dumps(payload).encode("utf-8")
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self.send_header("Cache-Control", "no-store")
                ...
                self.wfile.write(data)
                return
            if req_path.startswith("/pixhawk-mavlink-csv/download"):
                csv_path, download_name = pixhawk_mavlink_csv_recorder.download_csv()
                if csv_path is None:
                    data = json.dumps(
                        {"ok": False, "error": "no pixhawk mavlink csv session available"}
                    ).encode("utf-8")
                    self.send_response(404)
                    self.send_header("Content-Type", "application/json")
                    self.send_header("Cache-Control", "no-store")
        ...
        def do_POST(self):
            if (
                ...
                and not self.path.startswith("/pixhawk-mavlink-csv")
                and not self.path.startswith("/open-project-folder")
                and not self.path.startswith("/external-project")
            ):
            ...
            if req_path.startswith("/mode"):
                ...
                data = json.dumps({"ok": True, "status": status}).encode("utf-8")
                self.send_response(200)
            elif req_path.startswith("/pixhawk-mavlink-csv/start"):
                status = pixhawk_mavlink_csv_recorder.start()
                data = json.dumps({"ok": True, "status": status}).encode("utf-8")
                self.send_response(200)
            elif req_path.startswith("/pixhawk-mavlink-csv/stop"):
                status = pixhawk_mavlink_csv_recorder.stop()
                data = json.dumps({"ok": True, "status": status}).encode("utf-8")
                self.send_response(200)
            elif req_path.startswith("/open-project-folder"):
                target_dir = _EXTERNAL_MAP_PROJECT_PATH.expanduser().resolve()
                if not target_dir.exists() or not target_dir.is_dir():
                    data = json.dumps(
                        {
                            "ok": False,
                            "error": f"project directory not found: {target_dir}",
                        }
                    ).encode("utf-8")
                    self.send_response(404)
                else:
                    try:
                        ...
                        self.send_response(200)
            elif req_path.startswith("/external-project/run"):
                response_payload = external_project_runner.start()
                data = json.dumps(response_payload).encode("utf-8")
                self.send_response(200 if response_payload.get("ok") else 500)
            elif req_path.startswith("/external-project/stop"):
                response_payload = external_project_runner.stop()
                data = json.dumps(response_payload).encode("utf-8")
                self.send_response(200 if response_payload.get("ok") else 500)
            else:
                snap = state.snapshot()
...
def _init_mavlink_interface(pixhawk_cfg, use_barometer, use_mavlink=None):
    ...
    mav_csv_cfg = pixhawk_cfg.get("pixhawk_mavlink_csv", {})
    if not isinstance(mav_csv_cfg, dict):
        mav_csv_cfg = {}
    mav_csv_rate_hz = float(mav_csv_cfg.get("rate_hz", 20.0))
    default_csv_msgs = [
        "GPS_RAW_INT",
        "GLOBAL_POSITION_INT",
        "ATTITUDE",
    ...
    raw_csv_msgs = mav_csv_cfg.get("messages", default_csv_msgs)
    if not isinstance(raw_csv_msgs, list):
        raw_csv_msgs = default_csv_msgs
    mav_csv_messages = []
    for msg_name in raw_csv_msgs:
        msg_text = str(msg_name).strip().upper()
        if not msg_text:
            continue
        if msg_text.startswith("MAVLINK_MSG_ID_"):
            msg_text = msg_text[len("MAVLINK_MSG_ID_"):]
        mav_csv_messages.append(msg_text)
        ...
        for candidate_device in _iter_candidate_mavlink_devices(pixhawk_cfg):
            ...
            try:
                ...
                mavlink_interface.request_message_interval(
                    ...
                    rate_hz=mav_csv_rate_hz,
                )
                altitude_msg_id = getattr(mavutil.mavlink, "MAVLINK_MSG_ID_ALTITUDE", None)
                if altitude_msg_id is not None:
                    mavlink_interface.request_message_interval(
                        msg_id=altitude_msg_id,
                        rate_hz=mav_csv_rate_hz,
                    )
                vfr_hud_msg_id = getattr(mavutil.mavlink, "MAVLINK_MSG_ID_VFR_HUD", None)
                if vfr_hud_msg_id is not None:
                    mavlink_interface.request_message_interval(
                        msg_id=vfr_hud_msg_id,
                        rate_hz=mav_csv_rate_hz,
                    )
                if use_barometer:
                ...
                for msg_name in mav_csv_messages:
                    msg_id = getattr(mavutil.mavlink, f"MAVLINK_MSG_ID_{msg_name}", None)
                    if msg_id is None:
                        continue
                    mavlink_interface.request_message_interval(
                ...
                try:
                    mavlink_interface.master.mav.request_data_stream_send(
                        mavlink_interface.master.target_system,
                        mavlink_interface.master.target_component,
                        mavutil.mavlink.MAV_DATA_STREAM_ALL,
                        max(1, int(mav_csv_rate_hz)),
                        1,
                    )
                except Exception:
                    pass
...
def main():
    ...
    pixhawk_mavlink_csv_recorder = PixhawkMavlinkCsvRecorder(
        _repo_root() / "pixhawk_mavlink_csv_logs"
    )
    ...
    if mavlink_interface is not None:
        mavlink_interface.set_recv_listener(
            pixhawk_mavlink_csv_recorder.log_mavlink_message
        )
        imu_rate_hz = float(pixhawk_cfg.get("imu_rate_hz", vio_imu.IMU_RATE_HZ))
        mavlink_interface.request_message_interval(
            msg_id=mavutil.mavlink.MAVLINK_MSG_ID_HIGHRES_IMU,
    ...
    external_project_runner = ExternalProjectRunner(_EXTERNAL_MAP_PROJECT_PATH)
    ...
    if compass_mode == "pixhawk":
        compass_mode = "mavlink_compass"
    ...
    mav_csv_poll_cfg = pixhawk_cfg.get("pixhawk_mavlink_csv", {})
    if not isinstance(mav_csv_poll_cfg, dict):
        mav_csv_poll_cfg = {}
    mav_csv_poll_max_messages = max(
        1, int(mav_csv_poll_cfg.get("poll_max_messages_per_tick", 40))
    )

    def _poll_mavlink_capture_messages():
        if mavlink_interface is None:
            return
        for _ in range(mav_csv_poll_max_messages):
            msg = mavlink_interface.recv_match_safe(blocking=False)
            if msg is None:
                break
    ...
    def on_update(
        ...
        *_rest,
        ...
        _poll_mavlink_capture_messages()
        if vio_mode == "vio_imu":
            print_interval_s = pixhawk_cfg.get("print_interval_s", PRINT_INTERVAL_S)
        ...
        mavlink_direct_altitude_m = None
        vz_override_mps = None
        if mavlink_interface is not None:
            for message_name, field_name in (
                ("GLOBAL_POSITION_INT", "amsl"),
                ("ALTITUDE", "relative"),
                ("VFR_HUD", "vfr"),
                ("ALTITUDE", "amsl"),
                ("GLOBAL_POSITION_INT", "relative"),
            ):
                cached_alt_m = mavlink_interface.get_direct_altitude_m(message_name, field_name)
                if cached_alt_m is not None:
                    mavlink_direct_altitude_m = float(cached_alt_m)
                    break
        ...
        # Match the Pixhawk compass gauge shown in gui.html: attitude yaw in degrees.
        pixhawk_gauge_heading_deg = None
        if last_attitude["value"] is not None:
            yaw_rad = _safe_float(last_attitude["value"].get("yaw"))
            if yaw_rad is not None:
                pixhawk_gauge_heading_deg = (float(yaw_rad) * 180.0 / math.pi) % 360.0
        ...
        if active_output_mode == "gps_mavlink":
            gps_mavlink_mode.handle(
                ...
                yaw_deg=pixhawk_gauge_heading_deg,
            )
            ...
            optical_flow_gps_port_mode.handle(
                ...
                heading_deg=pixhawk_gauge_heading_deg,
        ...
        if dashboard_state is not None:
            ...
            if gps_origin is not None:
                ...
                vo_heading_deg = None
                vo_alt_m = (
                    _safe_float(mavlink_direct_altitude_m)
                    if mavlink_direct_altitude_m is not None
                    else (
    ...
    if use_vo_pipeline:
        ...
        try:
            ...
            pixhawk_mavlink_csv_recorder.stop()
