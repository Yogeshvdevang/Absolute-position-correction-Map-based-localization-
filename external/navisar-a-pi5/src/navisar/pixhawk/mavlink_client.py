...
class MavlinkInterface:
    ...
    def set_recv_listener(self, listener):
        """Set optional callback invoked for every received MAVLink message."""
    ...
    def recv_match_safe(self, **kwargs):
        ...
        try:
            msg = self.master.recv_match(**kwargs)
            if msg is not None:
                self._cache_message(msg)
                if self._recv_listener is not None:
                    try:
                        self._recv_listener(msg, time.time())
                    except Exception:
                        pass
            return msg
            ...
            # Guard transient parser/runtime failures from pymavlink so loops can continue.
            self._warn_recv_error(exc)
            return None
    ...
    def get_direct_altitude_m(self, message_name="GLOBAL_POSITION_INT", field_name="relative"):
        """Return the latest direct altitude estimate cached from MAVLink."""
        payload = self._last_direct_altitude.get((str(message_name), str(field_name)))
        if payload is None:
            return None
        return payload.get("altitude_m")
    ...
    def _cache_message(self, msg):
        """Cache selected message fields for fast non-blocking access."""
        msg_type = msg.get_type()
        ...
        for field_name, (attr_name, scale) in field_map.items():
            value = getattr(msg, attr_name, None)
            if value is None:
                continue
            self._last_direct_altitude[(msg_type, field_name)] = {
                "altitude_m": float(value) / float(scale),
                "time_s": time.time(),
