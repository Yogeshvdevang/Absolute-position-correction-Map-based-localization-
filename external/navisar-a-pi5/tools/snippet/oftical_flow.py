...
def main():
    ...
    # --- CSV logging setup ---
    csv_dir = Path(__file__).resolve().parent / "optical_flow_logs"
    csv_dir.mkdir(parents=True, exist_ok=True)
    run_ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    of_csv_path = csv_dir / f"optical_flow_sensor_{run_ts}.csv"

    of_csv_file = open(of_csv_path, "w", newline="", encoding="utf-8")
    of_csv_writer = csv.writer(of_csv_file)
    of_csv_writer.writerow([
        "timestamp",
        "sensor_time_ms",
    ...
    of_csv_file.flush()
    print(f"\n[CSV] Optical flow sensor data -> {of_csv_path}")
    ...
    def on_data(sensor_data):
        ...
        try:
            ...
            of_csv_writer.writerow([
                f"{time.time():.6f}",
                d["time_ms"],
                d["distance_mm"], f"{d['distance_cm']:.1f}", f"{d['height_m']:.4f}",
                d["dis_status"], d["strength"], d["precision"],
                d["flow_vx"], d["flow_vy"],
                d["flow_quality"], d["flow_status"],
                f"{d['speed_x']:.6f}", f"{d['speed_y']:.6f}",
            ])
            of_csv_file.flush()
        except Exception:
            pass
    ...
    try:
        ...
        try:
            ...
            pass
        print(f"[CSV] log saved: {of_csv_path}")
