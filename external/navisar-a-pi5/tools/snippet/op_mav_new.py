...
Sends:
  OPTICAL_FLOW     (#100)  — read by ArduPilot FLOW_TYPE=5, shows in opt_qua
  RANGEFINDER      (#173)  — legacy simple range output (distance, voltage)
...
class BootClock:
    """Monotonic boot-style clock for MAVLink time fields."""
    def __init__(self):
        self._start_monotonic = time.monotonic()

    def now_boot_ms(self):
        return int((time.monotonic() - self._start_monotonic) * 1000.0)

    def now_boot_us(self):
        return int((time.monotonic() - self._start_monotonic) * 1_000_000.0)

    def from_sensor_ms_to_boot_us(self, sensor_ms):
        return int(max(0, int(sensor_ms))) * 1000

    def from_sensor_ms_to_boot_ms(self, sensor_ms):
        return int(max(0, int(sensor_ms)))


# ─────────────────────────────────────────────────────────────
# MAVLink interface — sends OPTICAL_FLOW (#100) + RANGEFINDER (#173)
# ─────────────────────────────────────────────────────────────

class MavlinkInterface:
    def __init__(self, device, baud=115200, heartbeat_timeout=15.0,
                 source_system=1, source_component=192, boot_clock=None):
        self.device = device
        self.baud   = int(baud)
        self.boot_clock = boot_clock if boot_clock is not None else BootClock()
    ...
    def send_optical_flow(self, sample: OpticalFlowSample):
        ...
        """
        if sample.time_ms > 0:
            time_usec = self.boot_clock.from_sensor_ms_to_boot_us(sample.time_ms)
        else:
            time_usec = self.boot_clock.now_boot_us()
        h          = sample.height_m
        ...
        try:
            ...
            if self.flow_sent % 200 == 0:
                print(f"[MAVLink] flow_sent={self.flow_sent}  rangefinder_sent={self.rangefinder_sent}")
        except Exception as e:
            print(f"[MAVLink] ⚠ optical_flow_send error: {e}")

    def send_rangefinder(self, sample: OpticalFlowSample, range_min_m=0.01):
        """
        Send RANGEFINDER message (#173): distance [m], voltage [V].
        """
        if sample.dist_ok:
            distance_m = sample.distance_mm / 1000.0
        else:
            distance_m = range_min_m

        try:
            self.master.mav.rangefinder_send(
                float(distance_m),  # distance [m]
                0.0,                # voltage [V]
            )
            self.rangefinder_sent += 1
        except Exception as e:
            print(f"[MAVLink] ⚠ rangefinder_send error: {e}")

    def close(self):
        self._running = False
        self._hb_thread.join(timeout=2)
        self.master.close()
        print(f"[MAVLink] Closed. flow_sent={self.flow_sent}  rangefinder_sent={self.rangefinder_sent}")
...
class OpticalFlowBridge:
    def __init__(self, send_interval_s=1.0, print_enabled=False,
    ...
    def handle(self, now, sample, mav: MavlinkInterface):
        ...
        if now - self._last_send < self.send_interval_s:
            return
        # Do not resend the same sample at a faster loop frequency.
        current_obj_id = id(sample)
        if sample.time_ms > 0:
            if sample.time_ms == self._last_sample_time_ms:
                return
        else:
            if current_obj_id == self._last_sample_obj_id:
                return

        mav.send_optical_flow(sample)
        mav.send_rangefinder(sample, self.range_min_m)

        if self.print_enabled:
            print(f"[Bridge] flow_vx={sample.flow_vx} flow_vy={sample.flow_vy} "
                  f"quality={sample.flow_quality} dist_cm={sample.dist_cm:.1f}")
        self._last_send = now
        self._last_sample_time_ms = sample.time_ms
        self._last_sample_obj_id = current_obj_id
...
def parse_args():
    ...
    p.add_argument("--flow-port",          default="/dev/ttyAMA2")
    ...
    p.add_argument("--send-interval-s",    type=float, default=1.0,
                   help="How often to send MAVLink messages (default 1.0 = 1 Hz)")
